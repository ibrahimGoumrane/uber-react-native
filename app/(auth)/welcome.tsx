import CustomButton from "@/components/customButton";
import { onboarding } from "@/constants/index";
import { router } from "expo-router";
import { useRef, useState } from "react";
import { Text, TouchableOpacity, View, Image } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import Swiper from "react-native-swiper";
export default function OnBoarding() {
  const swiperRef = useRef<Swiper>(null);
  const [activeIndex, setActiveIndex] = useState(0);
  const isLastSlide = activeIndex === onboarding.length - 1;
  return (
    <SafeAreaView className="flex h-full items-center justify-between bg-white">
      <TouchableOpacity
        onPress={() => {
          router.replace("/(auth)/sign-up");
        }}
        className="flex w-full justify-end items-end p-5"
      >
        <Text className="text-black text-md  font-JakartaBold">Skip</Text>
      </TouchableOpacity>
      <Swiper
        ref={swiperRef}
        loop={false} //this line is used to stop the loop
        dot={<View className="w-8 h-1 mx-1 bg-[#E2E8F0]" />}
        activeDot={<View className="w-8 h-1 mx-1 bg-[#0286ff]" />}
        onIndexChanged={(index) => setActiveIndex(index)}
      >
        {onboarding.map((item) => {
          return (
            <View
              className="flex items-center justify-center p-5"
              key={item.id}
            >
              {item.image ? (
                <Image
                  source={item.image}
                  className="w-full h-72"
                  resizeMode="contain" //this line is used to fit the image in the container
                />
              ) : null}
              <View className="flex flex-row items-center justify-center w-full mt-10 ">
                {item.title ? (
                  <Text className="text-black text-3xl font-bold mx-10 text-center">
                    {item.title}
                  </Text>
                ) : null}
              </View>
              {item.description ? (
                <Text className="text-lg font-JakartaSemiBold  text-center text-[#858585] mx-10 mt-3">
                  {item.description}
                </Text>
              ) : null}
            </View>
          );
        })}
      </Swiper>
      <View className="flex items-center justify-center w-2/3 mb-8">
        <CustomButton
          title={isLastSlide ? "Get Started " : "Next"}
          onPress={() => {
            if (isLastSlide) {
              router.replace("/(auth)/sign-up");
            } else {
              swiperRef.current?.scrollBy(1);
            }
          }}
          className=""
        ></CustomButton>
      </View>
    </SafeAreaView>
  );
}
